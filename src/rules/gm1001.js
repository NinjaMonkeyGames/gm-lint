/**
 * @file Rule GM1001: Ensures that the `continue` keyword is only used within an enclosing loop statement.
 * @module rules/gm1001
 */

/**
 * Metadata for the GM1001 rule.
...
/** @type {Set<string>} GML keywords that define a valid loop context for `continue`. */
const LOOP_KEYWORDS = new Set(['for', 'while', 'repeat', 'with', 'do']);
 ...
 * @property {boolean} isLoop - Whether this scope sits inside a valid loop context for `continue`.
 ...
 * @property {boolean} isLoop - Whether the pending construct qualifies as a valid loop context.
...
function evaluateToken(token, stack, parenDepth, issues) 
{
  ...
  if (token.value === 'continue' && !currentScope.isLoop) 
  {
    issues.push({
      line: token.line,
      column: token.column,
      message: 'GM1001: The "continue" keyword must be used inside an enclosing loop statement.',
    });
  }
  return null;
}
...
/**
 * Checks GML code for invalid usage of the `continue` keyword outside enclosing loops.
 ...
 * @returns {LintIssue[]} Array of detected GM1001 diagnostic issues.
 */
export default function checkGM1001(content, lines) 
{
  /** @type {LintIssue[]} */
  const issues = [];
